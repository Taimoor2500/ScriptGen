 ...
 */
export function createSession(userId, { db = getDb(), now = Date.now } = {}) {
 ...
 * @returns {{ user: object, session: { id: string, expires_at: number } } | null}
 */
export function readSessionFromRequest(req, { db = getDb(), now = Date.now } = {}) {
...
export function serializeSessionCookie(value, { maxAgeSec = SESSION_TTL_MS / 1000 } = {}) {
  ...
  if (!value) {
    // Clearing: expire immediately regardless of maxAgeSec.
